...
These papers compare performance of methods like naive bayes, KNN, and logistic regression. It's interesting but they give conflicting outcomes and recommendations. From both it seems like naive bayes can be a decent classifier. 

I think the challenge with these papers is there is no standardized way of comparing different approaches. Each paper uses a different data set with a suite of different data processing/cleaning methods. So, it's hard to glean anything informative from any given paper in this category. 
